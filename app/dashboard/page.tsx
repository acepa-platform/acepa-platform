  ...
  return (
    <main className="min-h-screen bg-[#f7f8fc] text-slate-950">
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-64 border-r border-slate-200 bg-white lg:flex lg:flex-col">
        <div className="flex h-20 items-center border-b border-slate-100 px-7"><Link href="/dashboard" className="text-2xl font-black tracking-[-0.07em]">ACEPA</Link></div>
      ...
      {mobileOpen && <div className="fixed inset-0 z-50 lg:hidden"><button aria-label="Close menu" className="absolute inset-0 bg-slate-950/40" onClick={() => setMobileOpen(false)} /><aside className="relative flex h-full w-72 flex-col bg-white shadow-2xl"><div className="flex h-20 items-center justify-between border-b border-slate-100 px-6"><Link href="/dashboard" className="text-2xl font-black tracking-[-0.07em]">ACEPA</Link><button onClick={() => setMobileOpen(false)} className="rounded-lg px-3 py-2 text-sm font-bold text-slate-500">Close</button></div><nav className="space-y-1 p-4">{navItems.map((item, index) => <Link key={item.label} href={item.href} onClick={() => setMobileOpen(false)} className={`flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-semibold ${index === 0 ? "bg-slate-950 text-white" : "text-slate-600 hover:bg-slate-50"}`}><span className="w-7 text-center">{item.icon}</span>{item.label}</Link>)}</nav><div className="mt-auto p-4"><button onClick={handleSignOut} disabled={signingOut} className="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm font-bold text-slate-700 disabled:opacity-60">{signingOut ? "Signing out..." : "Sign out"}</button></div></aside></div>}
      <div className="lg:pl-64">
        <header className="sticky top-0 z-30 border-b border-slate-200/80 bg-white/90 backdrop-blur-xl"><div className="flex h-20 items-center justify-between px-5 sm:px-8 lg:px-10"><button onClick={() => setMobileOpen(true)} className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-bold lg:hidden" aria-label="Open menu">☰</button><div className="hidden lg:block"><p className="text-xs font-semibold text-slate-400">ACEPA WORKSPACE</p><p className="mt-1 text-sm font-bold text-slate-800">Personal dashboard</p></div><div className="flex items-center gap-3"><button className="relative flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-600"><span className="text-lg">♢</span><span className="absolute right-2 top-2 h-1.5 w-1.5 rounded-full bg-purple-600" /></button><button onClick={handleSignOut} disabled={signingOut} className="flex h-10 items-center gap-2 rounded-xl border border-slate-200 bg-white px-2 pr-3 disabled:opacity-60"><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-slate-950 text-xs font-bold text-white">K</span><Link href="/profile" className="hidden text-sm font-bold text-slate-700 sm:block hover:text-purple-600">Profile</Link></button></div></div></header>
        <div className="mx-auto max-w-[1500px] px-5 py-8 sm:px-8 lg:px-10 lg:py-10">
          <section className="rounded-[2rem] bg-slate-950 p-7 text-white shadow-xl sm:p-9 lg:p-11"><div className="max-w-3xl"><p className="text-xs font-bold uppercase tracking-[0.22em] text-purple-300">Welcome to ACEPA</p><h1 className="mt-4 text-3xl font-black tracking-[-0.05em] sm:text-4xl lg:text-5xl">Create value. Find opportunity. Make progress.</h1><p className="mt-4 max-w-2xl text-sm leading-7 text-white/65 sm:text-base">Your personal space for discovering opportunities, participating in meaningful projects, tracking your activities, and building your progress across ACEPA.</p><div className="mt-7 flex flex-wrap gap-3"><Link href="/discover" className="rounded-xl bg-white px-5 py-3 text-sm font-bold text-slate-950 transition hover:bg-purple-50">Discover opportunities →</Link><Link href="#opportunities" className="rounded-xl border border-white/15 bg-white/5 px-5 py-3 text-sm font-bold text-white transition hover:bg-white/10">View recommendations</Link></div></div></section>
