    ...
    <UserAccountShell>
      <main className={"min-h-screen " + surface}>
        ...
        <div className="mx-auto max-w-[1500px] px-5 py-6 lg:px-8">
          ...
          {activeTab === "Account" && <section className={"mt-8 rounded-3xl border p-6 shadow-sm " + card}><p className="text-xs font-bold uppercase tracking-[0.16em] text-purple-600">Appearance</p><h2 className="mt-2 text-xl font-black">Choose your appearance</h2><p className={"mt-2 text-sm " + muted}>Choose how ACEPA should look across your devices.</p><div className="mt-5 grid gap-3 sm:grid-cols-3">{[["light","☀️","Light"],["dark","🌙","Dark"],["system","🖥️","Automatic / System"]].map(([value,icon,label]) => <button key={value} onClick={() => { setAppearance(value); localStorage.setItem("acepa-appearance", value); window.dispatchEvent(new CustomEvent("acepa-appearance-change", { detail: value })); }} className={"rounded-2xl border p-5 text-left transition " + (appearance === value ? "border-purple-500 bg-purple-50 text-purple-700 shadow-sm" : soft)}><span className="text-xl">{icon}</span><p className="mt-3 text-sm font-black">{label}</p><p className={"mt-1 text-xs " + muted}>{value === "light" ? "Bright and clean" : value === "dark" ? "Dark interface" : "Follow your device"}</p></button>)}</div><button onClick={saveAppearance} disabled={saving} className="mt-5 rounded-xl bg-slate-950 px-5 py-3 text-sm font-bold text-white disabled:opacity-60">{saving ? "Saving..." : "Save appearance"}</button></section>}
        </div>
      </main>
    </UserAccountShell>
  );
